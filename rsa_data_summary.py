...
#############################################
###  FOR PROCESSING SUMMARY DATA RECORDS  ###
#############################################
# class Data(object):
...
def type_21(df: pd.DataFrame) -> pd.DataFrame:
...
def type_30(df: pd.DataFrame) -> pd.DataFrame:
...
def electronic_count_data_type_30(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if data.empty:
        ...
        try:
            dir_1 = dfh.loc[dfh[0]=="L1",2].astype(int).min()
            dir_2 = dfh.iloc[dfh[0]=="L1",2].astype(int).max()
        ...
        if data[1].isin(["0", "2"]).any():
            ...
            direction = header.loc[header[0] == "L1", 1:3].astype(str)
            direction = direction.drop_duplicates()
...
def type_70(df: pd.DataFrame) -> pd.DataFrame:
...
def type_60(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if data.empty:
        ...
        length_bin_code = dfh[2]
        number_of_bins = dfh[3].max()
        ddf = data.iloc[:, 0:]
        ddf = pd.DataFrame(ddf).dropna(axis=1, how="all")
...
def process_dates_and_times(df: pd.DataFrame, date_length: int, time_length: int, duration_min: int) -> pd.DataFrame:
